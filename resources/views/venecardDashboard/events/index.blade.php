...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (eventTypeSelect && eventPackageDiv) {
                            ...
                            <div class="col-md-6">
                                <div class="card shadow-sm h-100">
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            <label for="contact_name" class="form-label">Contact Name <span class="text-muted">(optional)</span></label>
                                            <input type="text" class="form-control" id="contact_name" name="contactName"
                                                placeholder="Contact name">
                                        </div>

                                        <div class="mb-3">
                                            <label for="contact_phone" class="form-label">Contact Phone <span class="text-muted">(optional)</span></label>
                                            <input type="tel" class="form-control" id="contact_phone" name="contactPhone"
                                                placeholder="Contact phone">
                                        </div>

                                        <div class="mb-3">
                                            <label for="contact_email" class="form-label">Email Address <span class="text-muted">(optional)</span></label>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($events as $index => $event)
                            @php
                                $rowNumber = $events->currentPage() * $events->perPage() - $events->perPage() + $index + 1;

                                $eventCardPath = optional($event->eventcard)->card_name;

                                if ($event->card_name == 'sample card') {
                                    $eventCardPath = 'eventSampleCards/sample.jpg';
                                }

                                if (empty($eventCardPath)) {
                                    $eventCardPath = 'eventsCardSamples/sample card.jpg';
                                }

                                $eventCardPath = str_replace('\\', '/', $eventCardPath);
                                $eventCardPath = str_replace('/storage/', '', $eventCardPath);
                                $eventCardPath = str_replace('storage/', '', $eventCardPath);
                                $eventCardPath = str_replace('app/public/', '', $eventCardPath);
                                $eventCardPath = str_replace('public/', '', $eventCardPath);
                                $eventCardPath = ltrim($eventCardPath, '/');

                                $eventCardUrl = asset('storage/' . $eventCardPath);
                                $scanUrl = url('/scan-cards/' . $event->code);
                                $invitationReportUrl = url('/invitation-updates/' . $event->code);
                            @endphp
                            ...
                            <tr>
                                ...
                                <td>
                                    <img
                                        src="{{ $eventCardUrl }}"
                                        alt="{{ $event->name }} card"
                                        style="width: 60px; height: 80px; object-fit: cover; border: 1px solid #ddd;"
                                ...
                                <td>
                                    <div class="fw-semibold">{{ $event->code }}</div>

                                    <div class="d-flex gap-2 mt-1">
                                        @if($event->venue_map_location_link)
                                            <a
                                                href="{{ $event->venue_map_location_link }}"
                                                target="_blank"
                                        ...
                                        <button
                                            ...
                                            title="Copy scan link for {{ $event->name }}"
                                        >
                                        ...
                                        <button
                                            type="button"
                                            class="btn btn-sm btn-outline-secondary copy-btn"
                                            data-clipboard-text="{{ $invitationReportUrl }}"
                                            title="Copy invitation report link for {{ $event->name }}"
                                        >
                                ...
                                <td>
                                    @php
                                        $nameParts = explode(' ', optional($event->user)->name ?? 'N/A');
                                    @endphp
                                ...
                                <td>
                                    <span class="badge text-bg-primary">{{ $event->eventGuests->count() }}</span>
                                </td>
